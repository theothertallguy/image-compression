...
class SQtree {
 private:
  ...
  pair<int,int> splitPoint(stats & s, pair<int,int> & ul, int w, int h);
 ...
 private:
  ...
  PNG renderNode(Node * node, PNG im);

  /**
   * Private helper to recursively calculate the size of a node and its children
   * An initialized node has size 1, plus the size of it's children.
   * @param node is the node whose size is to be determined
  */
