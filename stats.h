...
class stats {
 ...
 private:
  ...
  long getVectRect(vector<vector<long>> vect, pair<int,int> ul, int w, int h);

  double getColVar(char channel, pair<int,int> ul, int w, int h);

  void setSums(PNG & im, int x, int y); 

  void setVectRect(vector<vector<long>> vect, pair<int,int> ul, int colVal);
